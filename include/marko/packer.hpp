...
protected:
    int size;
    union { byte b[NUM]; char c[NUM]; } buffer;
    // byte buffer[NUM];
};
...
//     void pack(const char* fmt...) // C-style "const char* fmt, ..." is also valid
//     {
//         va_list args;
//         va_start(args, fmt);
//
...
//     // std::string fmt;
//     int size;
//     std::map<char, int> fmts { {'c', 1}, {'B', 1}, {'f', 4}, {'d', 8}, {'i', 4}, };
//     union { std::byte b[num*sizeof(float)]; float f[num]; char c[num*sizeof(float)];} buffer;
// };
