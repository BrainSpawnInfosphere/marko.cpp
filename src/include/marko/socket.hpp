...
public:
  ...
  */
  int recvfrom(void *dst, int size, sockaddr_t &from_addr, const int flags=0) {
    ...
    // guard(recv_len, "recvfrom() failed"); // timeout gives -1 ... avoid needless throws

    return recv_len;
  }

  int sendto(const std::string &sdata, const sockaddr_t &addr) {
    int size = ::sendto(socket_fd, sdata.c_str(), sdata.size(), 0, (struct sockaddr *)&addr,
                  sizeof(addr));
    guard(size != sdata.size(), "sendto() sent incorrect number of bytes");
    return size;
  }

  int sendto(const void *data, int size, const sockaddr_t &addr) {
    int num = ::sendto(socket_fd, data, (size_t)size, 0, (struct sockaddr *)&addr,
                      sizeof(addr));
